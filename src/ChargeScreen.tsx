...
export function ChargeScreen({ patientId, onBack, admin }: Props) {
  ...
  const list = supplies.filter((s) => s.name.toLowerCase().includes(query.trim().toLowerCase()))
  ...
  function confirmLoad() {
    const batch = confirmChargeBatch(patientId, draft)
    if (!batch) return
    setDraft({})
    setConfirmOpen(false)
    setToast('Insumos cargados al historial')
    setView('historial')
  ...
  function copyPendingText() {
    ...
    navigator.clipboard.writeText(text).then(() => {
      setToast('Copiado. Pegalo en el sistema de la clínica.')
      setTimeout(() => setToast(''), 2200)
    })
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          onClick={onBack}
          className="min-h-11 rounded-xl border border-slate-200 px-4 font-medium text-slate-500 transition-colors hover:bg-slate-50"
        >
          ← Pacientes
        </button>
        {admin ? (
          <button
            ...
            className="min-h-11 flex-1 rounded-xl bg-slate-900 px-4 font-medium text-white transition-colors hover:bg-slate-800"
          >
        ...
        <>
          {admin ? (
            <p className="rounded-xl bg-amber-50 px-4 py-2.5 text-sm text-amber-800">
              Uso excepcional: cargá acá solo si necesitás cerrar al paciente y le preguntaste al
          ...
          <input
            ...
            className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-slate-900 outline-none transition-shadow focus:border-teal-600 focus:ring-2 focus:ring-teal-600/15"
          />
                ...
                return (
                  ...
                  >
                    <span className="text-[0.95rem] font-medium text-slate-800">
                      {s.name}
                      {s.favorite ? (
                        <span className="ml-2 text-[0.7rem] font-semibold uppercase tracking-wide text-teal-600/80">
                          frecuente
                        </span>
                      ) : null}
                    </span>
                    <div className="flex items-center gap-1 rounded-full bg-slate-100 p-0.5">
                      ...
                      <button
                        ...
                        className="grid h-10 w-10 place-items-center rounded-full bg-teal-700 text-lg font-medium text-white transition-colors hover:bg-teal-800"
                      >
                        +
                      </button>
                    </div>
          ...
          <div className="sticky bottom-3 pt-2" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
            <button
              type="button"
              disabled={!draftTotal}
              onClick={openConfirm}
              className="min-h-13 w-full rounded-xl bg-teal-700 font-semibold text-white shadow-lg shadow-teal-900/20 transition-colors hover:bg-teal-800 disabled:opacity-40 disabled:shadow-none"
            >
              {draftTotal ? `Cargar insumos · ${draftTotal}` : 'Cargar insumos'}
            </button>
          </div>
        </>
      ) : (
              ...
              <div className="mt-3 flex flex-col gap-3">
                {history.map((batch) => (
                  <div key={batch.id} className="rounded-xl bg-slate-50 p-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-xs font-medium ${
                            batch.transferred ? 'text-emerald-600' : 'text-slate-400'
                          }`}
                        >
                    ...
                    {batch.lines.map((l) => (
                      <div key={`${batch.id}-${l.supplyId}`} className="flex justify-between gap-3 py-1">
                        <span className="text-sm text-slate-600">{l.supplyName}</span>
                        <strong className="text-sm font-medium text-slate-900">× {l.quantity}</strong>
                      </div>
                    ))}
                  </div>
                ))}
      ...
      {confirmOpen ? (
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">
              Servicio <strong className="font-semibold text-slate-700">{service.name}</strong>. Al
              confirmar, queda en el historial del paciente.
            </p>
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <button
                ...
                className="min-h-12 flex-1 rounded-xl bg-emerald-600 font-medium text-white shadow-sm transition-colors hover:bg-emerald-700 disabled:opacity-40 disabled:shadow-none"
              >
                Confirmar y pasar
              </button>
            </div>
